...
- The **background layer** (`item.layer`, `'foreground'`/`'background'`) is a different concept from the z-order layering above — it's a Word header/footer-style *editing* toggle, not a stacking position. Exactly one layer is "active" at a time (`main.js`'s `backgroundEditMode`); `render.js`'s `setActiveLayer` makes the inactive layer's items dimmed and fully non-interactive (`evented: false`, not just unselectable), so drawing set-piece shapes never competes for clicks with the lighting layout, or vice versa. New items are created on whichever layer is active; existing items move between layers via the inspector's "Background" checkbox (`INSPECTOR_FIELDS`, not exempt from `locked` the way the checkbox itself is).
